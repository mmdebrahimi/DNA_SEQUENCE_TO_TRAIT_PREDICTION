...
Ledger v6 (2026-06-03): **v0 resolver SHIPPED + FROZEN** (deterministic VF-marker compatibility + abstention + provenance; CLI; H4 eval; cross-axis ExPEC rescue at honest **0.833 strategy-cap**; 30/30 tests). **EP-4 recall-metric optimization HALTED** — the 24-genome ExPEC(Salipante)-vs-EPEC(Hazen) cohort is **study==class confounded** (0.729-probe "batch not biology" commit 789d83d; k-mer baseline 0.514 chance), so further tuning measures lab-of-origin not biology. **Next action = COMPLETE v0 via the VF side-by-side diff** (last originating-goal deliverable; in_place minister mission `ep4-v01-vf-diff`, needs BLAST+/KMA install — run from a dna_decode session). THEN freeze v0; THEN the labels frontier = de-confounded (within-study / lineage-matched) ExPEC+EPEC set. NT/embedding track **DEFERRED** (tet precedent + confound; orphaned 3× on GTX 860M). H1 (label non-circularity) still under-investigation = the binding scientific constraint, not engineering.
