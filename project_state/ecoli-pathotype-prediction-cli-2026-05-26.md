...
| 14 | v0 cross-axis ExPEC rule external validity (N=1,209 independent isolation-source ExPEC, Horesh): recall ~0.53 (LOWER bound, detection-limited) vs in-sample 0.917; specificity ~0.99 vs intestinal EPEC (0.7%) / ETEC (0%). Rule is conservative (high specificity, modest recall); NOT re-tuned. | `research_outputs/expec_rule_external_validity_2026-06-04.md` + `scripts/expec_rule_external_validity.py` | high | 2026-06-04 |
...
| **H1 FALSIFIED for pathotype discrimination — de-confounded ExPEC+EPEC cohort is NOT buildable (intrinsic sampling confound)** | 2026-06-04 | Horesh F1 audit (`research_outputs/horesh-f1-label-provenance-audit-2026-06-04.md`, N=10,146): independent isolation-source ExPEC labels exist (1,574 blood/urine, 242 lineages) BUT ExPEC-vs-EPEC is confounded on ALL three control axes — source-study (shared pool 7+59), sampling context (blood/urine vs feces, 100% disjoint BY DEFINITION of the pathotype), phylogeny (5 shared lineages, 7+20 strains). The confound is intrinsic (label==sampling-context), not a collection gap. Prior 24-genome k-mer 0.514=chance was a correct null. **Consequence:** the v0.1 embedding-classifier upgrade for pathotype is a dead-end on available labels (any AUROC measures sampling context); v0 deterministic resolver is the honest tool. Embedding-upgrade frontier should target sampling-INDEPENDENT lab-measurement labels (AMR MIC / quantitative assays), NOT sampling-defined categories. |
...
| RESOLVED 2026-06-04: **Horesh 2021 File F1 supplementary audit** (per-record label-provenance distribution) | claude | requires download + parse of `https://pmc.ncbi.nlm.nih.gov/articles/PMC8208696/` File F1 supplementary | DONE — `research_outputs/horesh-f1-label-provenance-audit-2026-06-04.md`. Curated/independent fraction = 20.5% (< 70% → H1 FAIL on full collection); `(predicted)` circular = 52.2%. ExPEC isolation-source labels independent (1,574) but ExPEC-vs-EPEC intrinsically sampling-confounded → no de-confounded discrimination cohort buildable. Horesh usable as a clade-diverse ExPEC specificity set (H4), NOT a discrimination AUROC substrate. |
...
Ledger v7 (2026-06-04): **v0 pathotype resolver SHIPPED + FROZEN + PACKAGED** — deterministic VF-marker compatibility + abstention + provenance + the **VF side-by-side diff** (canonical blastn over the same DB; the last originating-goal spec item, shipped via `/soraya minister ep4-v01-vf-diff`, mission-mvp-reached). Installable as `dna-pathotype` (console script, pure-stdlib); README section + git tag `pathotype-v0`; full repo suite 789 passed; committed 9ab5960 + pushed to main. ExPEC recall at honest **0.833 cross-axis cap**. **The binding constraint is now LABELS, not models** — every available pathotype label is either circular (EnteroBase = BlastFrost gene-calls; NCBI PD = AMRFinder-derived) or study==class confounded (the 24-genome ExPEC/EPEC cohort: k-mer 0.514 = chance), so the classifier/embedding track (v0.1) CANNOT be honestly validated yet. **Next epoch = the labels-independence frontier (H1):** Horesh 2021 File F1 provenance audit -> within-study / lineage-matched ExPEC+EPEC cohort -> classifier falsifier (does an embedding model beat the resolver on INDEPENDENT labels?). Label circularity is the recurring meta-constraint across the whole trait-decoding roadmap, so it is built as a reusable de-confounding capability, not a one-off. NT/embedding track stays DEFERRED until non-circular labels exist.
...
| 16 | 2026-05-30 | research | [Soraya advance runs 1135+1807] ETEC accessions ENUMERATED from von Mentzer supplementary (downloaded Springer CDN XLSX + openpyxl parse) | 8 reference chromosome accessions resolved (E925→LR883050 … via MOESM5) into etec_reference_vonmentzer_strains CSV. BONUS: 558-genome by-accession ETEC GCA collection extracted from MOESM4 → etec_vonmentzer_collection_gca_2026-05-30.csv. ETEC now the best-resourced slice arm, fully by-accession. Toxin-circularity caveat unchanged (near-conformance only). openpyxl added to venv. |
| 17 | 2026-05-31 | write-plan | [Soraya planning] Two-machine capability map + transfer-minimization long-term plan | plans/Two_Machine_Capability_Map_And_Transfer_Minimization_Plan_2026-05-31.md. Thesis: erase capability asymmetry → EP-4 becomes fully laptop-local. Laptop has Docker 29.4.3 + CUDA(860M) + D: 4.4TB free; VF/blastn is CPU-replicable. 2 moves to transfer-free: A1 (laptop installs VF+blastn+DB on D:, solo) + A2 (workhorse one-time commits pathotype_horesh_* scripts to git). Data never transferred — re-derived from public accessions. AMR/NT track's only scale-out is Databricks cloud, not workhorse. |
| 18 | 2026-06-03 | edit-local-code | ExPEC recall hardened: per-gene ExPEC support scoring (RULE_EXPEC_002) in dna_decode/pathotype/{markers,resolve,expec_score}.py. **SUPERSEDED by row 19** — the minister fast-path shipped flat-K=1 (recall 0.917) which over-rescued capsule-only JSPG; reverted to the user's committed cross-axis rule. | (superseded) flat-K=1 gave 0.917 but rescued JSPG on a lone serum-resistance traT gene — the overfit the user pre-authorized refusing. See row 19. |
| 19 | 2026-06-03 | edit-local-code | ExPEC recall hardened (CROSS-AXIS, user round-2 commitment): rescue requires BOTH axes — >=1 iron-acquisition gene AND >=1 capsule/serum gene (meets_cross_axis_support in expec_score.py; resolve_call cross_axis_support param). Structurally excludes capsule-only JSPG. [Soraya minister run 2026-06-03-1309-ep4-v01-expec-recall corrected per project_state/ep4-v01-expec-recall/round2-commitments-corrected.md] | ExPEC recall 0.75 -> **0.833 (10/12) = blocked:strategy-cap** (the committed "clean 0.833 beats overfit 0.917"). Rescues JSMY (5 iron + traT serum, genuine cross-axis); JSPG EXCLUDED (capsule-only, iron=0); JSLG AMBIGUOUS (empty, 0 support — unrescuable without weakening the strong-marker rule, no P-fimbriae lever in cache). Confident-supported precision 1.0; EPEC recall 1.0 (LEE-gate above support branch; Q3 LEE+full-cross-axis synthetic -> EPEC regression test added). Gate: tests/test_pathotype_expec_recall.py 7/7 PASS (incl JSPG structural-exclusion + overfit-canary); full pathotype suite 30/30. Scope-limit: cross-axis tested in-sample on N=24. |
| 20 | 2026-06-03 | stop | EP-4 recall-metric optimization HALTED (substrate confounded); v0 near-complete (ONE spec item left = VF side-by-side diff) | Soraya analysis (two independent sessions converged): the ExPEC-vs-EPEC 24-genome cohort is study==class confounded (ExPEC=Salipante, EPEC=Hazen) — confirmed by the 0.729-probe ("batch not biology", commit 789d83d) + k-mer baseline 0.514 (chance). Further recall/AUROC tuning on it has ~0 info value (measures lab-of-origin, not biology). **Sequence locked:** (a) COMPLETE v0 via the VF side-by-side diff (the last originating-goal deliverable; in_place minister mission, needs BLAST+/KMA install) -> (b) THEN freeze v0 as shipped+scope-capped -> (c) THEN the labels frontier = de-confounded (within-study / lineage-matched) ExPEC+EPEC set. NT/embedding track DEFERRED (tet precedent + confound; orphaned 3x on GTX 860M). NOT chosen now: ETEC 3-class expansion (plasmid-toxin full-GCA fetch), re-opening confound. |
| 21 | 2026-06-04 | edit-local-code | VF side-by-side diff shipped: dna_decode/pathotype/vf_runner.py (canonical blastn VirulenceFinder caller over the same VF DB) + vf_diff provenance section wired into cli.py; BLAST+ 2.17.0 installed native (Q1 ratified); honesty: caller_is_independent_baseline=false + same-DB non-independence caveat (Q2 ratified) — audit of the fast k-mer-seed caller, not independent validation; tests/test_pathotype_vf_diff.py (4 tests, offline-safe degradation). [Soraya minister run 2026-06-04-0008-ep4-v01-vf-diff, in_place; 2 interrogation receipts ratified via AskUserQuestion] | PASS — 4/4 vf_diff tests green (incl real-blastn STX2 both_present agreement, concordance 1.0); full pathotype suite 31 passed; completes the v0 originating-goal spec item. |
| 22 | 2026-06-04 | propose | /project-state --update-last-evaluation | v0 COMPLETE — VF side-by-side diff shipped (mission-mvp-reached); full repo suite 789 passed; next epoch = freeze v0 scope-capped, then de-confounded-labels frontier. |
| 23 | 2026-06-04 | edit-local-code | v0 FROZEN + PACKAGED: pyproject version 0.0.1->0.1.0 + `dna-pathotype` console-script entry; README pathotype-resolver section; git tag pathotype-v0; committed 9ab5960 + pushed to main. | v0 is shippable-tool state (FASTA-in CLI verified end-to-end; dna-pathotype command works). |
| 24 | 2026-06-04 | propose | /project-state --refresh-frame | current-state: v0 pathotype resolver SHIPPED + FROZEN + PACKAGED; binding constraint is now LABELS not models; next epoch = labels-independence frontier (H1) = Horesh F1 audit -> within-study ExPEC+EPEC cohort -> classifier falsifier. |
| 25 | 2026-06-04 | research | Horesh 2021 File F1 label-provenance audit (download F1 10,146-record metadata CSV from Figshare 13270073 + per-record provenance/isolation-source/lineage analysis) | H1 FALSIFIED for pathotype discrimination — de-confounded ExPEC+EPEC cohort INFEASIBLE (intrinsic sampling confound: label==isolation-context on all 3 control axes). Independent ExPEC isolation-source labels exist (1,574) but not for discrimination. Memo: research_outputs/horesh-f1-label-provenance-audit-2026-06-04.md. |
| 26 | 2026-06-04 | propose | /project-state --update-hypothesis H1 + --resolve-pending-decision (Horesh F1) | H1 -> falsified (for pathotype discrimination); Horesh F1 pending decision resolved; classifier/embedding track stays closed for pathotype (was conditional on H1 PASS). |
| 27 | 2026-06-04 | run-tests | v0 cross-axis ExPEC rule external-validity check on Horesh independent labels (scripts/expec_rule_external_validity.py, N=1,574) — closes the markers.py "K=1 in-sample on N=24" scope-limit | recall ~0.53 (lower bound) vs in-sample 0.917; specificity ~0.99 vs intestinal EPEC/ETEC; rule kept conservative + NOT re-tuned (refused-overfit discipline); scope-limit updated in markers.py + memo. |
...
- **Date:** 2026-06-04
- **Progress signal:** v0 COMPLETE — VF side-by-side diff shipped (vf_runner.py canonical blastn caller + vf_diff provenance section; BLAST+ 2.17.0 native; same-DB audit-not-independent honesty per T1/Q2; offline-safe; 4 tests). Closes the last v0 originating-goal spec item via `/soraya minister ep4-v01-vf-diff` (mission-mvp-reached). Full repo suite 789 passed. Earlier: ExPEC recall hardened to honest 0.833 cross-axis cap (row 19) + recall-tuning HALTED (cohort study==class confounded, k-mer 0.514, row 20). Next epoch: freeze v0 scope-capped, then the de-confounded-labels frontier (research/substrate, not modeling).
