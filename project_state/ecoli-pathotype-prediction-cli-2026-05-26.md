...
| 4 (new) | **NCBI Pathogen Detection host_disease facet query** — query NCBI Pathogen Detection isolate browser SOLR index to facet-count E. coli isolates by `host_disease` field; estimate Tier-3 substrate density for ExPEC/UPEC + commensal | research | claude | ≤1 week | open |
| 5 | Cross-check audit-gate reuse: can EP1 SUSPEND-gate scaffolding (mechanism × phenotype merge pattern) be repurposed for pathotype-call opacity? Define pathotype-specific opacity (aggR-alone, partial LEE, contradictory toxin/regulator hits) | research | claude | ≤2 weeks | open |
...
| **NEW: NCBI Pathogen Detection host_disease facet query** (E. coli isolate-browser SOLR index) | claude | requires NCBI Pathogen Detection API/SOLR query | Confirms whether host_disease field is densely populated enough for E. coli to serve as Tier-3 substrate for ExPEC/UPEC + commensal expansion. |
<!-- project-state:end:pending-decisions -->

## Bellman-Inspired Decision Frame

### Current state (one-line summary)
Ledger v3 (2026-05-27): /research substrate-survey complete (Mission Control L1 run); 4-tier substrate strategy LOCKED (Tier-1 DECA + von Mentzer + prototypes / Tier-2 Horesh 2021 pending File F1 audit / Tier-3 NCBI host_disease / Tier-4 EnteroBase clonal-lineage only); architecture-fork FINAL LOCK = Candidate 5 (deterministic multilabel cluster resolver + abstention); v0 output contract drafted with 23 markers + 11-class decision table; H1 status "under-investigation" pending Horesh File F1 audit + Whittam direct-contact follow-up; 3 Short-term actions completed (substrate-survey, T1+T2 resolution, architecture-fork lock), 5 open (Horesh audit, Whittam contact, VF integration, NCBI host_disease facet, audit-gate reuse).
