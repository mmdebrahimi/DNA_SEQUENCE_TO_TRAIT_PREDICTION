...
| 14 | 2026-09-11 | edit-local-code | Soraya --advance: closed the cell_evidence_line wiring shortfall named in ledger row 10 -- 11 of 44 routes were wired, the other 33 carried measured registry evidence that reached no user. | WIRING IS NOW EXHAUSTIVE: 41 routes wired, 3 DELIBERATELY excluded, and the exclusions are the interesting half. The 14 mammal coat-colour routes took ONE edit because mammal_color_cli derives the route (f'dna-{organism}color') instead of hand-listing it -- a new organism cannot ship unwired. THE THREE EXCLUSIONS WERE MEASURED, NOT ASSUMED: dna-amr already prints a per-drug trust_block line (ciprofloxacin 'INDEPENDENT_MEASURED -- acc 0.95 (N=16007)') plus four disclosure layers, and the generic route-level line would print 'NOT_CENSUSED (59 cells, weakest shown) -- shipped but never censused' DIRECTLY BENEATH it -- a self-contradicting human-facing disclosure, the lenacapavir defect exactly, and wiring it would have been the THIRD instance. dna-decode-forward prints method-specific measured evidence (BLOSUM62 vs learned) and dna-decode-inverse prints its own 'evidence: ESM beats a no-oracle null' line; both are dna-decode SUBCOMMANDS with no console script. So all 3 already disclose MORE specifically than the generic line. GUARD STRENGTHENED FROM A COUNTER TO AN EXACT SET: the old test asserted only that SOME routes were missing, which a newly-shipped unwired route would have satisfied silently; it now asserts missing == the named exclusion set, so a new route fails BY NAME until wired or given a written reason, and a second test pins that each excluded route really does still disclose (an exclusion whose justification decayed would otherwise sit quietly in a comment). The route->module map is DERIVED from pyproject rather than guessed from the route name -- the documented dna-pneumo-serotype/pneumoserotype trap that once made the wiring a silent no-op. VERIFIED ON REAL CLI RUNS, not just tests: rabbit/roedeer/guineapig/cat/coat/horse/pigeon/plumage/pigment/morphology/essentiality/metabolic/motility all print the line; 4 apparent zero-prints were MY bad arguments, not wiring failures, and were re-run clean. 92 tests in the guard file, non-vacuity proven by removing one route's wiring (fails by name). Suite 4646 -> 4711 pass / 0 fail. Frozen AMR surface byte-unchanged. |
| 15 | 2026-09-11 | research | Soraya --advance (spillover): closed candidate row 3 -- do the doubt/lineage lines belong on `dna-decode` routes too? | ANSWERED NO, BY MEASUREMENT, and the answer is structural rather than a preference. `trust_block` is keyed on (drug, organism) -- the AMR cell key -- and a non-AMR cell has no drug, so all six DISCLOSURE_LAYERS are unreachable for it: across 56 non-AMR cells, ZERO produce ANY layer. Wiring a doubt or lineage line into a colour / morphology / motility route would ship a branch that can NEVER FIRE, which is exactly what the doubt_layer decision already refused once (do not render a line that could never apply; make its ABSENCE detectable instead). Shipped as a TRIPWIRE, not a coverage test: it fails the moment any non-AMR cell acquires a layer, so the wiring question reopens on evidence rather than on memory. Proven NON-VACUOUS by a companion test asserting dna-amr still DOES carry layers -- without it the tripwire would pass trivially if trust_block ever stopped returning anything. Also corrected the guard file's own stale docstring, which still described the wiring as PARTIAL and pointed at a coverage counter that row 14 replaced with an exact-set assertion. 94 tests. No production code changed -- this candidate resolved to a measured negative. [done:5f0d511d] |
| 16 | 2026-09-12 | run-tests | Soraya --advance: closed candidate row 5 -- the ECOFF-anchored tiering evaluation, end to end, against a bar frozen before any carriage number existed. | VERDICT WEAK_DIRECTIONAL (one of four pre-named outcomes), which the bar requires be reported as NOT ESTABLISHED rather than a partial win. THE BIGGER RESULT IS THAT MOST OF THE QUESTION COULD NOT BE ASKED: 3 of 4 drugs were unscoreable before any comparison -- ciprofloxacin (ECOFF 0.06 vs Oxford panel floor 0.125) and ceftriaxone (0.125 vs 0.5) are DEGENERATE_ECOFF_BELOW_PANEL with 100% of isolates non-wild-type, and tetracycline has no MIC column at all. Only gentamicin was SCOREABLE. On it: clinically-S AND ECOFF-NWT carriage 0.080 (2 of 25) vs clinically-S AND ECOFF-WT 0.0045 (12 of 2652) -- an 18-FOLD relative enrichment in the predicted direction -- but the gap 0.0755 sits INSIDE its own permutation null (max 0.1159). Correct call, not a quibble: at n=25 against a 0.45% base rate one extra carrier moves the rate 0.04, so chance lands 3 routinely, and a bar that passed this would pass noise. CONTROL THAT MAKES THE NULL INTERPRETABLE: clinically-R carriage is 0.922, so the deployed rule demonstrably works on this cohort -- had that been low the flat susceptible strata would have indicted the RULE, not the anchor. THE DETERMINANT DEFINITION DECIDED THE RESULT AND WAS NEARLY WRONG: keying on Class=AMINOGLYCOSIDE would have pulled in 3,298 STREPTOMYCIN rows against 343 gentamicin ones, yielding a streptomycin co-carriage finding wearing a gentamicin label; the deployed rule's SUBSTRING Subclass match was replicated exactly. Zero rmt/npmA rows exist here, independently reconfirming the 2026-09-03 Oxford probe and making the v2 rescue inert. BAR DESIGN: a permutation null rather than an effect size, because at an unknown stratum size any threshold would have been arbitrary -- the mis-specification that sank two earlier bars this session. Honest limit: n=25/2 carriers means this cohort CANNOT ANSWER the question; that is not the same as answering no. The substrate fix is a full low-end dilution series, not a bigger cohort. 31 tests. No frozen file touched; mic_tiers.py stays sha256-pinned and adoption remains a user-authority call. [done:66af76f3] |
