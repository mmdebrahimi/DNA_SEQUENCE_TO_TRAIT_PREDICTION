...
G0 + G0-completion DONE (caller validated on real C. auris ERG11 alleles, 3/3). Both authority gates resolved (workhorse=Precision 7780; compute=local 12GB, PlantCAD_l32 fits). G1 INFRASTRUCTURE complete (scripts/build_fungal_cohort.py, 16 fungal tests green, validated on real sequence) — G1's only remaining input is a per-isolate [accession, fluconazole_mic, clade] label TSV (NCBI metadata + BV-BRC both dry; supplementary-only). Path B unblocked + teed up on the workhorse, held until G1 result is shared.
