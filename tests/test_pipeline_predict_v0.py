...
def test_load_audit_verdict_can_fallback_to_cohort_gate(tmp_path: Path):
    from scripts.pipeline import _load_audit_verdict
    ...
    v = _load_audit_verdict(merge_json, "EXTERNAL_SAMPLE", fallback_to_cohort_gate=True)
    assert v is not None
    assert v["noise_class"] is None
    assert v["cohort_gate_verdict"] == "SUSPEND_CONDITION_4"
    assert v["suspend_gate_fired"] is True
    assert "external genome is not present" in v["verdict_explanation"]


def test_load_audit_verdict_can_fallback_to_clean_cohort_gate(tmp_path: Path):
    from scripts.pipeline import _load_audit_verdict
    ...
    v = _load_audit_verdict(merge_json, "S_UNKNOWN", fallback_to_cohort_gate=True)
    assert v is not None
    assert v["cohort_gate_verdict"] == "RUN_FULL_AND_CLEAN"
...
def test_markdown_handles_external_sample_cohort_only_audit():
    from scripts.pipeline import _render_predict_markdown
    ...
    md = _render_predict_markdown(r)
    assert "Cohort gate verdict" in md
...
def test_embed_live_gene_sequences_batches(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        "dna_decode.models.foundation.model_factory",
        lambda model_name, config_path=None: FakeModel(),
    )

    gene_sequences = {f"g{i}": "ATGC" for i in range(9)}
    out = pipeline._embed_live_gene_sequences("mock", tmp_path / "cfg.yaml", gene_sequences)

    assert len(out) == 9
    assert [len(chunk) for chunk in calls] == [4, 4, 1]
