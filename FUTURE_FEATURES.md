...
**Status 26/08/2026 — the animal branch shipped and hit a wall worth reading before extending it.** The animal-morphology stage exists as 19 curated coat-colour / plumage cells, and the family is now **frozen at 19** (`dna_decode/data/colour_cell_freeze.py`). A catalog-derived screen found the blocker is *curation before substrate*: 40 of 65 loci record no causal variant at all (7 of 19 cells record none for any locus → unvalidatable as written, no cohort helps), and 14 of the 25 that do are indel/structural → off-panel for any SNP array. Any further constrained-morphology cell should be screened against gates **G9/G10** (`wiki/negative_results_map_2026-06-13.md`) *before* it is built — that is the check this family reached 19 cells without. Memo: `wiki/colour_cell_substrate_screen_2026-08-26.md`.
