...
- **Eukaryotic Path B (Arabidopsis flowering-time embedding test, Gate G2) is GPU-workhorse work, pre-staged on the laptop.** Full executable spec: `plans/EP8_PathB_PreStage_Manifest.md` (REVISED post-brainstorm 2026-06-08 — phenotype-AGNOSTIC window primary, NOT curated loci, to honor the "no curated catalog" embedding-niche criterion; CPU-only §0.5 dry-manifest gate; paired-bootstrap PASS CI; kinship/PCs primary de-confounder, geography sensitivity-only). The dry-manifest gate is CODED + unit-tested at `scripts/g2_dry_manifest.py` (`tests/test_g2_dry_manifest.py`); phenotype labels committed at `data/arabidopsis/`; `scripts/fetch_arabidopsis_pathb.py` pulls the genotype. Handoff: `wiki/HANDOFF_workhorse_eukaryotic_2026-06-07.md`. Ledger: `project_state/eukaryotic-trait-decoding-cycle-2026-06-07.md`. The GPU embedding run is the ONLY remaining step + lives on the personal Precision 7780 (NOT the Bombardier/DLP machine).
