"""Score the AR-Bank N. gonorrhoeae cell from the Kaggle AMRFinder factory output (no local Docker).

The Kaggle kernel `neisseria-amrfinder` SRA-assembles + AMRFinder-scans the AR-Bank N. gonorrhoeae
isolates (the assembly path that wedges Docker locally) and emits one `<biosample>.amrfinder.tsv` per
isolate. This scorer pulls those, parses the Element symbols, runs the NON-FROZEN neisseria_amr cell
(call_ng_cefixime + call_ng_ciprofloxacin), and scores vs the CDC
S/I/R labels -- a NEW-species AR-Bank validation (mixed SRA-only + assembly isolates).

  # 1. pull the Kaggle output (when the kernel finishes):
  uv run python scripts/kaggle_push_poll.py pull neisseria-amrfinder data/raw/ar_bank_gono_kaggle
  # 2. score:
  uv run python scripts/score_enterococcus_from_kaggle.py --amrfinder-dir data/raw/ar_bank_gono_kaggle
"""
...
DRUGS = {
    "cefixime": ("data/raw/ar_bank_neisseria_gonorrhoeae_extval_cefixime", NG.call_ng_cefixime),
    "ciprofloxacin": ("data/raw/ar_bank_neisseria_gonorrhoeae_extval_ciprofloxacin", NG.call_ng_ciprofloxacin),
}
...
def main() -> int:
    ...
    ap.add_argument("--amrfinder-dir", default="data/raw/ar_bank_gono_kaggle",
                    help="dir of Kaggle-produced <biosample>.amrfinder.tsv files")
    ap.add_argument("--min-per-class", type=int, default=5)
    ap.add_argument("--run-id", default=f"gono_kaggle_{_date.today().isoformat()}")
    a = ap.parse_args()
    ...
    if not amr_dir.exists():
        print(f"REFUSE: AMRFinder dir {amr_dir} not found -- pull the Kaggle output first "
              f"(kaggle_push_poll.py pull neisseria-amrfinder {amr_dir})")
        return 2
    ...
    artifact = {
        "_schema": "ar-bank-gono-kaggle-validation-v1", "date": _date.today().isoformat(),
        "run_id": a.run_id, "organism": "Neisseria gonorrhoeae",
        ...
        "genotype_source": ("Kaggle-native bioconda SRA-assembly (skesa) + AMRFinderPlus "
                            "-O Neisseria_gonorrhoeae; local Docker-wedge path bypassed"),
    ...
    out = Path(f"wiki/ar_bank_gono_kaggle_validation_{a.run_id}.json")
