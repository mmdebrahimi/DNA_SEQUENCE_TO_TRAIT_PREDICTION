"""HIV INSTI v0.1 — a MUTANT-SPECIFIC integrase-inhibitor catalog data-derived from HIVDB fold-associations.

Mirrors the SHIPPED NRTI/PI v0.1 arc (`scripts/hiv_{nrti,pi}_mutant_catalog.py`) for the 5 INSTIs
(RAL/EVG/DTG/BIC/CAB). The position-based v0 (`INSTI_CLASS`, any non-consensus at a major integrase
position 66/92/118/138/140/143/147/148/155/263) over-calls benign polymorphisms + accessory mutations
that ride on resistant lineages (the `hiv_insti_baseline_vs_ols` memo bounds the headroom: OLS beats the
position catalog by RAL +0.069 / DTG +0.076 / CAB +0.314, ties EVG -0.026 / BIC -0.011). v0.1 fix: keep
only the amino-acid variants ACTUALLY resistance-associated in the data. A naive carriers'-median-fold
rule is CONFOUNDED (an accessory mutation co-occurs with a major one -> its carriers' median fold is high
though it isn't independently resistant), so the resistant set is the mutants whose MULTIVARIATE OLS
log10-fold coefficient >= log10(1.5) (an independent >=1.5x effect after controlling for co-occurrence;
>= MIN_CARRIERS carriers). That deconfounds the accessory/polymorphic positions.

DELTA-HONEST CUTOFF (same as the PI builder): there is NO per-drug clinical INSTI cutoff sourced in-repo,
so BOTH the v0 position-based call AND the v0.1 mutant-specific call are scored at the SAME UNIFORM
illustrative fold>=3 boundary (`ILLUSTRATIVE_FOLD_CUTOFF`). The headline is the v0->v0.1 balanced-accuracy
GAIN at that fixed cutoff (a fair refinement signal), NOT an absolute-calibration claim.

CIRCULARITY GUARD: the catalog is derived FROM the fold data, so the headline metric is 5-fold
CROSS-VALIDATED (derive the resistant-mutant set on the training folds, evaluate on the held-out fold) ->
an HONEST out-of-sample estimate. The fixed deliverable catalog (derived on all data) is written separately.

HONEST EXPECTATION (the deferral this CLOSES): INSTI headroom is mixed, NOT uniform like PI — RAL/DTG have
real positive deltas, EVG/BIC are ~flat (v0.1 can't beat a near-optimal position call), and CAB's large
delta is at n=64 (UNSTABLE). v0.1 is expected to improve-or-hold the positive-delta drugs and hold the
flat ones; a per-drug result, not a uniform win.

Label = PhenoSense fold (independent wet-lab; NOT HIVDB Sierra). DATA: data/raw/hiv/INI_DataSet.txt
(gitignored; cite Rhee 2003).
"""
...
DEFAULT_DATA = REPO / "data" / "raw" / "hiv" / "INI_DataSet.txt"
...
def derive_resistant_mutants(records) -> set[str]:
    """Resistant mutants at the major INSTI positions via MULTIVARIATE OLS (controls for co-occurrence).
...
def run(path: Path = DEFAULT_DATA) -> dict:
    cls, _, drug_cols = _CLASS_SPECS["INSTI"]
    ...
    return {
        "artifact": "hiv_insti_mutant_specific_v0_1", "schema": "hiv-insti-mutant-catalog-v0_1",
        ...
        "cutoff_note": "DELTA-HONEST: both v0 (position-based) and v0.1 (mutant-specific) scored at the SAME "
                       "UNIFORM illustrative fold>=3 (no per-drug INSTI clinical cutoff sourced in-repo); the "
        ...
        "honest_caveats": [
            "DELTA-HONEST at the uniform fold>=3 cutoff (no per-drug INSTI clinical breakpoint sourced); the "
            "GAIN is the signal, not the absolute balacc",
            "deconfounded (multivariate-OLS-coefficient) derivation, mirroring the shipped NRTI/PI v0.1 arc; "
            ...
            "INSTI headroom is MIXED, not uniform like PI: RAL/DTG have positive OLS-vs-catalog deltas, "
            "EVG/BIC are ~flat (position call near-optimal), CAB's large delta is at n=64 (UNSTABLE). v0.1 "
            "is expected to improve-or-hold positive-delta drugs + hold the flat ones; a per-drug result.",
...
def render_md(result: dict, generated: str) -> str:
    lines = [f"# HIV INSTI v0.1 - mutant-specific catalog (data-derived, CV held-out) ({generated})", ""]
    ...
    lines.append("")
    lines.append(f"Position-based v0 over-calls accessory/polymorphic riders at the major integrase "
                 f"positions; v0.1 keeps only fold-associated mutants. Both metrics are 5-fold CV held-out "
                 f"(out-of-sample). **{result['n_drugs_improved_or_held']}/{result['n_drugs']} INSTI drugs "
...
def main(argv=None) -> int:
    ...
    out_md = args.out_md or (REPO / "wiki" / f"hiv_insti_v0.1_validation_{today}.md")
