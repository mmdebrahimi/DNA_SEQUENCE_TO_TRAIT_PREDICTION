...
- **MILESTONE 2026-06-05 (cipro, the first DE-CONFOUNDED Phase-2 falsifier):** on the cipro N=147 cohort (the only substrate passing the de-confound gate — 6 shared R/S MLST lineages, country+year non-aliasing), **NT-XGBoost AUROC 0.914 vs k-mer-XGB 0.824 = +8.9 pp** under leakage-safe `leave_one_accession_out` CV (deterministic, reproduced 4×). **SCOPE:** this beats the bag-of-k-mers SEQUENCE baseline, NOT yet "best classical / architecture validated" — the QRDR-POINT *knowledge* baseline (AMRFinder gyrA/parC/parE mutation features; gene-presence is useless since R+S both carry gyrA) is computing (AMRFinder backfill ~50/147). Paired-bootstrap CI + NT-vs-POINT come from the final `amr_falsifier --amrfinder-runs --skip-kmer` run. Single drug+cohort ⇒ NO architecture-class promotion. New infra: `dna_decode/eval/cohort_deconfound.py` (de-confound gate, a hard precondition), `scripts/amr_falsifier.py` (drug-agnostic, CI-aware), `dna_decode/eval/point_baseline.py` (QRDR-POINT comparator). Per 3× /brainstorm.
