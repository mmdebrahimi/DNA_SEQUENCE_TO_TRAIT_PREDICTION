# Provenance-disjoint validation — Klebsiella x meropenem — 2026-06-10

Deployed decoder `call_resistance(organism=Klebsiella, drug=meropenem)` scored on a PROVENANCE-DISJOINT NCBI-PD subset (submitters OUTSIDE NARMS/CDC/FDA/GenomeTrakr/PulseNet/USDA).

## Result

| metric | value |
|---|---|
| n scored | 60 (TP 14 FP 3 TN 27 FN 16; abstain 0) |
| accuracy | 0.683 |
| sensitivity (R) | 0.467 |
| specificity (S) | 0.9 |
...
## Leakage control
All 60 accessions are FRESH — excluded 799 accessions used in ANY prior klebsiella_* cohort (registry calibration + prior validation), so the score is on strains never seen in tuning or earlier validation.
