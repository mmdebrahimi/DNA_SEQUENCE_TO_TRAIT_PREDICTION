...
| **MaveDB** | `api.mavedb.org/api/v1` (search + scores) | **2796 score sets** (~all `protein_coding`; ~38 % ≤400 aa tractable) | JSON score sets; HGVS `hgvs_pro`/`hgvs_nt` variants | **Adapter built** (`mavedb_cpu_smoke.py` — DNA→protein translate + HGVS parse; mism=0). `/scores` endpoint intermittently 504 | — (chance rho on the imputed pilot scoreset; needs a real-score scoreset) |
