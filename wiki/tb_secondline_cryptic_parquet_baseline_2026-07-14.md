# TB second-line CRyPTIC-parquet baseline — moxifloxacin (+ bedaquiline in flight) (2026-07-14)
...
- **moxifloxacin — DONE** (this memo).
- **bedaquiline — in flight** (Rv0678/atpE/pepQ — a mechanistically DISTINCT new-drug cell; 2448 grade-1/2 determinants). Artifact commits when the stream completes.
- **The remaining panel is one command each** (the wiring supports them): `uv run python scripts/score_tb_cryptic_parquet.py --drug {levofloxacin,amikacin,kanamycin,linezolid,ethionamide,clofazimine,delamanid,ethambutol}` — each ~15 min streaming the 2.9 GB parquet.
