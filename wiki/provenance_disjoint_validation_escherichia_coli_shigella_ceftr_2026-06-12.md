# Provenance-disjoint validation — Escherichia_coli_Shigella x ceftriaxone — 2026-06-12

Deployed decoder `call_resistance(organism=Escherichia_coli_Shigella, drug=ceftriaxone)` scored on a PROVENANCE-DISJOINT NCBI-PD subset (submitters OUTSIDE NARMS/CDC/FDA/GenomeTrakr/PulseNet/USDA).
...
## Leakage control
All 60 accessions are FRESH — excluded 859 accessions used in ANY prior escherichia_coli_shigella_* cohort (registry calibration + prior validation), so the score is on strains never seen in tuning or earlier validation.
