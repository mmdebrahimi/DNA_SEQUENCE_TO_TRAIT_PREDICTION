...
- **Archaea** — **truthful CLOSED NEGATIVE.** No usable label source: the substantial archaeal data (Haloferax/M. maripaludis transposon fitness, Sulfolobus forward genetics) is the wrong regime; archaeal AMR has no CLSI breakpoints and no catalog; trait DBs (Madin) are taxon-confounded (G3/G7). ~0 archaeal DMS in ProteinGym/MaveDB (confirm via the ProteinGym reference CSV). Only regime-compatible path = **acquiring one archaeal extremophile-protein DMS**.
