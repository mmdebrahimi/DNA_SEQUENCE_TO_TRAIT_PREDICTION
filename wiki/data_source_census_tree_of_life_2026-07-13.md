...
- **Bacteria** — richest new-cell vein. **TB second-line (CRyPTIC)** is the single lowest-friction win (same pipeline + WHO catalogue already in-repo). **N. gonorrhoeae** is the strongest new-genus candidate. **S. pneumoniae (GPS)** is huge but conditional — must filter to the *measured*-MIC subset (default PBP→MIC is G1). H. pylori has the cleanest single-SNP catalog but N is fragmented. Enterococcus vanco is a gene-presence identity (low novelty).
  - **Vibrio — CENSUS-MISSED candidate (added 2026-07-14; this census originally had ZERO Vibrio coverage).** *Vibrio parahaemolyticus/cholerae/alginolyticus* carry the **same gyrA S83/D87 + parC S80 QRDR** the decoder already validates for cipro, and marine/seafood *Vibrio* surveillance provides **measured** MIC/disk AST paired to deposited genomes → **clears the usual label-killers (G1 measured-not-tool-derived, G3 lab-assay, G5 deposited assemblies)**. Two verified FREE public BioProjects: **`PRJNA1254427`** (Korean seafood *V. parahaemolyticus*, CLSI MIC incl. cipro/tet, ~10 WGS+MIC) + **`PRJNA645603`** (Canadian imported-shrimp *Vibrio*, Kirby-Bauer disk, 4 isolates). **The binding constraint here is POWERING, not the label gate**: both are underpowered vs the ≥20/class bar (10 + 4). Path to power: pool multiple *Vibrio* measured-MIC BioProjects (candidate SRA sets `PRJNA699735`/`PRJNA1107692`/`PRJNA1155317` — verify per-isolate measured AST first) OR a larger unpublished marine-lab collection. So Vibrio cipro is a **clears-gates-but-underpowered new-cell candidate pursuable partly for FREE** — no acquisition strictly required for a small pilot. Provenance: `research_outputs/marine-vibrio-genome-mic-accessions-2026-07-14.md`.
