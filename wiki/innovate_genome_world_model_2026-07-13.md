...
### g5-catalog-extension  *(G5 relax "must beat the catalog")*
**Move.** Relax the assumed goal *"the learned model must BEAT the catalog"* → the learned arm's job is to **EXTEND** the catalog: find its blind spots (catalog-negative resistant cases) and propose candidate new entries — an active catalog-**curation** loop, not a competitor. This turns the executed negative (ESM ties the catalog only on the blind-spot subset, AUROC 0.449) from a dead end into a *job description*.
**Kill-test (executed).** DEAD iff the catalog has no blind spots to extend → HIV NNRTI catalog-negative resistant count == 0. Committed: **53** → SURVIVED. `[grounded]` 53 blind-spot isolates; `[grounded]` learned arm cannot BEAT but the blind-spot SET is real, findable work.
...
## Handoff
The synthesis (g9+g1+g5+g8) is the recommended reframe for the genome world model. The natural first build is **g5-catalog-extension** (it is the smallest, uses only committed data + the shipped catalog, and directly monetizes the executed HIV/ESM negatives) or **g8-residual-detector** (reuses shipped de-confounding machinery). Both are `/technical-plan`-ready. The two unfalsified ideas each need one new artifact before they can be scored. Read-only over committed artifacts; frozen decoder surface byte-unchanged (verify_lock OK).
