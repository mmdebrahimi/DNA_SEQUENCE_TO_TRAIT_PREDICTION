...
| `cohort_manifest.build_manifest()` is the data-driven leakage source-of-truth; `prior_accessions(exclude_cohort=name)` excludes by EXACT-self identity, NOT substring; `Manifest.incomplete` forces fail-closed | `dna_decode/eval/cohort_manifest.py` + `scripts/provenance_disjoint_validate.py` | Productionized 2026-06-10 (Anchor-4). Scans EVERY `data/raw/*/selected.tsv` + `data/processed/*.parquet` (744 accessions across all 8 parquet cohorts) — replaces the hardcoded `_FLAGSHIP_PARQUET_COHORTS` list (3 cohorts, ~175 accessions). EXACT-self identity forces a re-run of the same organism×drug onto fresh accessions (the old substring rule allowed reuse of an earlier provdisjoint set). `incomplete=True` on ANY load failure → `provenance_disjoint_validate.py` prints `INCOMPLETE_MANIFEST` + exits 2 unless `--allow-incomplete-manifest` stamps `manifest_degraded=True` into the artifact. An incomplete leakage registry is a possible false-independence claim, not a harmless offline fallback. |
| `ncbi_pd_provenance_census.py` self-persists its powering verdict + REFUSES to overwrite a good verdict with an error/row-capped row | `scripts/ncbi_pd_provenance_census.py` + `wiki/provdisjoint_census_results.json` | 2026-06-10. Previously stdout-only; now idempotently upserts one row per `(organism, drug)` (group→organism normalizer). The refusal-to-persist-degraded rule means a degraded re-run can't silently downgrade a previously-good powering verdict. This JSON is the SCORED-cell powering source the report card reads. |
| Decoder-suite validation report card rows = the deployed-claim surface ∪ observed cells; honest per-cell tier, NO aggregate headline | `scripts/build_validation_report_card.py` + `dna_decode/data/shipped_decoder_surface.py` | Productionized 2026-06-10 (Anchor-4). Read-only roll-up (exit 0 always — a report, not a gate); writes `wiki/decoder_validation_report_card.{md,json}`. Rows derive from the checked-in DEPLOYED-CLAIM surface (`SHIPPED_DECODER_SURFACE`) UNION observed scored/census keys, so a shipped decoder that's never been censused still renders (NOT_CENSUSED) — a new decoder can't ship invisibly (`tests/test_shipped_decoder_surface.py` asserts every CLI-routable drug has ≥1 surface row). 7 cell-states incl. LABEL_CONFOUNDED (oxacillin×S.aureus, unreliable mecA surrogate) + NO_FREE_PHENOTYPE_SOURCE (surface-driven, fungal/antiviral/antimalarial). Current (re-derived from the committed artifact, rebuilt 2026-08-24 — supersedes a stale "25 cells / 6 SCORED / 4 NOT_CENSUSED" figure that UNDER-stated the validated surface): **27 cells — 10 SCORED / 3 UNDERPOWERED / 2 ABSTAINS_BY_DESIGN / 1 LABEL_CONFOUNDED / 11 NO_FREE_PHENOTYPE_SOURCE / 0 NOT_CENSUSED**. The empty NOT_CENSUSED bucket is the meaningful part: every shipped decoder on `SHIPPED_DECODER_SURFACE` has been censused, so none renders invisibly. Re-derive with `uv run python scripts/build_validation_report_card.py` rather than trusting this sentence. |
| Lineage-disclosure clustering is GREEDY-representative (chaining-resistant), NOT `phylogeny.cluster_by_ani` single-linkage; lineage AUGMENTS the report card, never demotes the 7-state machine | `dna_decode/eval/clonality.py` + `scripts/compute_lineage_metrics.py` + `build_validation_report_card.py` | Added 2026-06-11. The report card's raw sens/spec counts one vote per ISOLATE → an over-sampled clone carries the metric (every SCORED R class is clonally dominated, ≤17 effective lineages at Mash 0.001). `clonality.py` does greedy clustering (pick a rep, drop everything within `threshold` of it, repeat) — single-linkage union-find would CHAIN A~B~C into one cluster. Mixed-label clones are DISCORDANT (never majority-voted; excluded from sens/spec + counted); `cluster_weighted_confusion` collapses each same-label lineage to one vote. `compute_lineage_metrics.py` recomputes per cohort: M1 genome-completeness gate (a cohort that can't complete is `partial` + emits NO lineage tier), M4 raw reconciliation (recompute `call_resistance` + assert raw == committed artifact BEFORE trusting any weighted number, else `ReconcileMismatch`), greedy clustering @ {0.001, 0.005}, cluster-weighted sens/spec each with Wilson CI + effective-N (C3 emitter guard: `_assert_weighted_renderable` REFUSES a weighted point without its CI — weighted N is tiny so the CI IS the point), graded lineage bucket. Sidecar `wiki/provdisjoint_lineage_metrics.json` (schema `provdisjoint-lineage-metrics-v1`), keyed by `dna_decode/data/cell_key.canonical_cell_key` (the SINGLE join key across all 3 sidecars), checkpointed per-cohort (Docker-wedge-safe). The Mash call lives ONLY in `compute_lineage_metrics.main()` (Docker); all math is pure + offline-testable. Tests: `tests/test_clonality.py` (20) + `tests/test_compute_lineage_metrics.py` (18). |

| The animal colour/plumage family is FROZEN at 19 cells; every colour cell's declared gate + promotion path is DERIVED from the live catalogs, never re-typed | `dna_decode/pigment/substrate_screen.py` + `dna_decode/data/colour_cell_freeze.py` + `tests/test_colour_cell_freeze.py` | Added 2026-08-26. `substrate_screen` derives each cell's scoreability from the committed catalogs; `colour_cell_freeze.FROZEN_COLOUR_ROUTES` is the literal roster (the deliberate friction point) and the test cross-checks it against the LIVE catalogs + `cli.TRAITS` so the hand-listed roster cannot drift — a hand-enumerated list beside the data that defines it is the repo's third-instance bug class. Screen findings: 40 of 65 loci (62%) record no causal variant, leaving 7 of 19 cells (alpaca, cattle, mouse, pig, pigeon, rabbit, sheep) **unvalidatable as written** (no cohort helps — the blocker is curation); of the 25 loci that do record one, 14 (56%) are indel/structural and off-panel for any SNP array. Only donkey (3/3) + roe deer (1/1) are fully SNV-tractable. All 19 `CellContract.incoming_data_gate` values now name the real gate (bare ids `G9`/`G10`, or `n/a` for the two that cleared) and 7 `demotion_rule` strings were corrected — they had promised "MEASURED needs a free \<species\> cohort" when no cohort could ever score a rule whose causal variants are unrecorded. **HONESTY: this is an attention/scope freeze, NOT enforcement** — it makes adding a 20th cell FAIL A TEST, which a determined edit can ratify by updating the literal. Do not describe it as a hard invariant. Gates G9/G10 in `wiki/negative_results_map_2026-06-13.md`; memo `wiki/colour_cell_substrate_screen_2026-08-26.{md,json}`. |
