...
_TRAIT_CONTRACTS: list[CellContract] = [
    ...
    CellContract(
        ...
        validation_slice=(
            "*** CURRENT NUMBER (2026-09-09, wiki/salmserovar_name_table_2026-09-09.md): wet-lab "
            "accuracy 0.8400 (168 hit / 11 miss / 21 no-call) on 200 isolates, vs SeqSero2 1.3.2 at "
            "0.8800 -- DELTA -0.0400, improved from -0.1750 across TWO changes today. *** STRATIFIED "
            "BY RESOLUTION ROUTE (wiki/salmserovar_resolution_strata_2026-09-09.{md,json}) -- the "
            "headline SURVIVES: 165 of 168 hits (98.2%) are UNIQUELY resolved, only 3 rest on an "
            "arbitrary tie-break, and the O+H1 phase-incomplete fallback produced ZERO calls (it was "
            "REACHED on 6 isolates and failed to collapse, so the zero is a measurement not an "
            "unexercised path). Tie-break-independent accuracy = 0.8250. RECORDED AGAINST IT: those 3 "
            "hits are bought by converting 5 abstentions into CONFIDENTLY-WRONG calls (hit rate 3/8 "
            "within that stratum), which is the losing side of the asymmetry THIS CELL pre-registered "
            "on 2026-09-04 ('an abstention is recoverable by a human, a confident wrong serovar is "
            "not') -- an OPEN acceptance-bar question, deliberately not self-resolved. Largest single "
            "no-call cluster = 4 isolates on ONE formula, 4:i:- (monophasic Typhimurium), blocked "
            "because 8 serovars share O=4/H1=i; named as the next lever, NOT taken. *** (1) THE O "
            "PROCEDURE (wiki/salmserovar_o_fix_result_2026-09-09.{md,json}) took it to 0.8250. *** (2) "
            "THE NAME TABLE then took it to 0.8400: build_table kept the FIRST serovar per (O,H1,H2) "
            "key and SILENTLY DISCARDED 213 serovars -- Cubana, Muenchen and Hvittingfoss were absent "
            "entirely, and its comment claiming 'subspecies-I named serovars come first' is falsified "
            "by its own data. It now applies SeqSero2's own remove_list + rename_dict (Virginia-> "
            "Muenchen, Hindmarsh->Bovismorbificans) + bracket expansion (r,[i] matches a bare r) + a "
            "subspecies-I tie-break that is OURS, not SeqSero2's. +3 hits, ZERO regressions -- but an "
            "INDEX-MATCHED CONTROL shows +1 of that is the scoring function (the same table feeds "
            "load_formula_index), so the CALLER-ATTRIBUTABLE gain is +2. *** BOTH changes were SHIPPED "
            "AS RECORDED OVERRIDES of their own frozen bars, and that pattern is itself the finding: "
            "the first bar used a NET no_call ceiling to police a DIRECTIONAL failure mode; the second "
            "set a +4 hit threshold from an un-analysed count when only 3 of the 5 cases are "
            "recoverable without guessing. Both bar files and both REJECT verdicts are left UNEDITED. "
            "Re-sizing an acceptance bar is an AUTHORITY call and both should be ratified or reversed "
            "deliberately. *** KNOWN REMAINING: the Agoueve/Cubana collision (13:z29:-) is wrong on 2 "
            "isolates and is unresolvable without subspecies input; the shipped ambiguity policy keeps "
            "an ARBITRARY winner on a contested formula (the principled abstain-instead variant was "
            "measured and is strictly WORSE: 165 hits, 3 regressions). *** The O axis was "
            ...
            "no-call) vs NCBI-PD's PUBLISHED in-silico field (`computed_types`) at 0.925 (184/15/1) on "
            "the SAME isolates. THAT FIELD IS A PRODUCTION CALL OF UNDOCUMENTED TOOL/VERSION, NOT a "
            ...
            "the reference METHOD, not merely an undocumented pipeline. *** THE ENTIRE GAP IS THE O "
            "AXIS: *** per-axis agreement with SeqSero2 is H1 200/200 = 1.000 and H2 146/146 = 1.000 "
            "where both resolved, against O 159/200 = 0.795 -- the H axes and the formula lookup are "
            "not the problem. Splitting O: 159 agree / 33 RESOLVED-BUT-DIFFERENT / 8 unresolved, so it "
            "is mostly CONFIDENTLY WRONG calls rather than abstentions (resolution 0.960, accuracy "
            "where resolved 0.828). 28 of those 33 (85%) are TWO strings -- `9,46`->`9` x15 and "
            "`1,3,19`-> various x13 -- and `1,3,19` CORROBORATES the independent 2026-09-04 "
            "observation that Typhi was called `1,3,19` rather than `9,12`, reached by a different "
            "route. A LEAD, NOT A DIAGNOSIS: the concentration names where to look, not why. "
            "QUOTE THE DELTA, NOT THE LEVELS: SeqSero2 itself scores 0.880 here against its own "
            "published ~0.95, because the cohort caps each serovar at 12 and so over-weights rare, "
            "harder serovars -- that depresses EVERY caller. Also observed, deliberately NOT "
            "explained: NCBI `computed_types` (0.920) BEATS the standalone reference tool (0.880) on "
            "these isolates. Prior figures retained for provenance -- DELTA "
            "-0.222, and a 29.5% abstention rate. IMPROVED 2026-09-04: the O-antigen COVERAGE cut was "
            "lowered 80->40 against a bar registered BEFORE the sweep (adopt iff >=7 rescued AND <=2 "
            "newly wrong AND net >=+5); measured 36 rescued for exactly 1 new error, net +35. Accuracy "
    ...
    CellContract(
        ...
        demotion_rule=(
            ...
            "half: H 0.7844 -> 0.9461, gain +0.1617; O -0.0068 (the same small cost, replicated). 28 H "
            "calls genuinely differ between the rules, so the comparison is POWERED -- the script "
            "refuses with UNDERPOWERED_RULES_NEVER_DIFFERED at 0 flips rather than reading a vacuous "
            "agreement as survival. The gain did NOT shrink this time, so lineage overlap was not "
            "carrying the earlier result; do NOT read that as 'the fix is bigger than we thought' -- "
            "different test sets (250 vs 169), not estimates of one quantity. ST is a lineage unit, NOT "
            "a clonality guarantee (stronger than isolate-disjoint, weaker than a phylogenetic split); "
            "2 isolates without a complete 7-locus ST are EXCLUDED and counted, never pooled. "
            "Re-score if the allele DB changes. The SAME coverage-primary "
