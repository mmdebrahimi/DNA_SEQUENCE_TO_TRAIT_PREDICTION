...
def _hit_matches_drug(hit, drug: str) -> bool:
    """True iff the determinant's Class/Subclass matches the drug's AMRFinder class set.

    Mirrors amr_rules.cipro_determinants_from_main's matching (case-insensitive
    substring on the '/'-joined multi-class strings AMRFinder emits).
    """
...
def _phenotype_for_feature(high_hits, drug_verdicts: dict, drugs: list[str]) -> list[dict]:
    ...
    For each HIGH-confidence determinant hit on the feature, attach a phenotype
    entry per matched drug (ABSTAIN-aware via the genome-level verdict). A
    determinant that matches no supplied drug still surfaces (drug=None,
    DETERMINANT_PRESENT) — it IS a curated resistance determinant.
    """
    out: list[dict] = []
    for jh in high_hits:
        matched = [d for d in drugs if _hit_matches_drug(jh.hit, d)]
        if matched:
            for d in matched:
            ...
            # Curated determinant present but no genome-level verdict drug matched.
            out.append({
                "drug": None,
