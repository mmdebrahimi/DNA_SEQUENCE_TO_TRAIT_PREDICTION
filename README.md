...
| `dna-hla` (**new**) | **human HLA drug-hypersensitivity** — carriage of clinically-actionable HLA alleles via validated tag SNPs, from a VCF (GRCh38): HLA-B\*57:01/**abacavir**, B\*58:01/allopurinol, A\*31:01/carbamazepine | HLA-allele carriage + CPIC drug action (avoid-drug) + tag genotype + provenance; LD-proxy (not sequence-based typing) | tag-SNP LD proxy (the deployed clinical screen — B\*57:01/rs2395029 sens ~100%); 1000G per-superpopulation AF corroborates the tag (SAS 0.087 > EUR 0.041 > AFR 0.003, tracking known B\*57:01 structure); sample-level concordance vs free 1000G HLA truth = pending (`scripts/hla_concordance.py`). Research-use, **NOT a clinical tool** |
