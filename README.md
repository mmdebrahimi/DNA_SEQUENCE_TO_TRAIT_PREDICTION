...
dna-decode decode my_genome.fna --run   # -> actually RUN the auto-runnable decoders, one report
```

It sniffs the input kind (nucleotide/protein FASTA or VCF), lists every applicable decoder with its
one-line claim + honest evidence tier, and prints the command to run each. Add **`--run`** to execute the
auto-runnable decoders in one report (genome -> the `profile` suite; protein -> `inverse`), reporting the
...
| `dna-clinvar` (**new**) | **human germline pathogenicity** — curated ClinVar (P/LP + B/LB) over the ACMG SF v3.2 + carrier 86-gene panel, from a VCF | per-carried-variant classification + gene / disease / gold-star review level; not-in-panel → INDETERMINATE (absence ≠ benign) | deterministic curated-catalog decoder (faithful-to-ClinVar); **deployment-demonstrated on real open-consent PGP-UK individuals** (N=5, 0 reportable pathogenic = expected ACMG-SF base rate; `wiki/clinvar_pgp_uk_cohort_card_2026-07-06.md`). Sibling of the `dna-pgx` human PGx cells (CYP2C19/2C9/2C8/3A5/TPMT/CYP2B6/**CYP2D6** + VKORC1/SLCO1B1). Research-use, **NOT a clinical tool** |
| `dna-pgx` | **human pharmacogenomics** — star-allele diplotype + CPIC metabolizer phenotype, from a phased VCF (GRCh38); 9 genes: CYP2C19/2C9/2C8/3A5/**CYP2D6**/TPMT/CYP2B6 + VKORC1/SLCO1B1 | diplotype + PM/IM/NM phenotype (or function readout) + per-variant genotypes + provenance; non-core → withheld/mis-called (documented) | GeT-RM consensus core-diplotype concordance on real 1000G, independent of the consensus tools: CYP2C19 72/72 · CYP2C9 73/73 · CYP2C8 82/82 · TPMT 85/85 (compound *3A) · CYP2B6 62/62 · CYP3A5 8/8 (underpowered) · **CYP2D6 46/47 SNP surface** (`wiki/pgx_getrm_concordance_cyp2d6_2026-07-06.md`). **CYP2D6 STRUCTURAL surface** (read-depth copy-number off a real BAM/CRAM, `dna_decode.pgx.cyp2d6_structural`): *5 deletion + *xN duplication **26/26** on real 1000G CRAMs (`wiki/cyp2d6_structural_2026-07-06.md`; hybrid identity = Cyrius-class v0.3). Trio-Mendelian QC across all 7 star genes. **NOT a clinical tool** |
| `dna-decode inverse` (**new**) | **effect → EDIT** — inverse design: propose the edits at a target **percentile of predicted molecular damage** (Regime B: enzyme fitness/stability), from a protein sequence (+ optional CDS → single-nt-reachable edits only). Uses the DMS-validated forward oracle as **label-free ground truth** — no phenotype label is consulted | k proposed edits + codon changes + each one's score-percentile, plus an explicit `does_not_support` list and a tie-group warning when the scorer is coarsely quantized | **It RANKS, it does NOT dose.** Graded **non-circularly** against MEASURED wet-lab ProteinGym DMS (calibrate/select on disjoint POSITION splits; grade on the proposed variant's measured value, never the model's own re-score). **Floor vs ceiling, both measured at scale** (`wiki/esm_at_scale_2026-07-17.md`): the **SHIPPED wheel-only default (`blosum62`) does NOT generalize** — across N=200 ProteinGym it beats a random pick materially on only **13.5%**, often worse than guessing — while the **LEARNED method (ESM2-650M, GPU/precomputed table) beats the null on 72.9%** (N=188). So the real capability is real and general but lives in the learned method; the default is a cheap first pass, and **utility does not track forward rank** (PTEN 0.5185 earns its keep, RL40A 0.5190 does not) → run the falsifier on *your* protein first. The magnitude version PASSES on blaTEM (+53.0%) but is **not deployable** (needs the target protein's own DMS; calibrators cannot transfer). Regime B only — **never** clinical resistance (use `dna-decode amr`). Research use only |
| `dna-hla` (**new**) | **human HLA drug-hypersensitivity** — HLA-B\*57:01 carriage → **abacavir** hypersensitivity risk, via the tag SNP rs2395029, from a VCF (GRCh38) | HLA-allele carriage + CPIC drug action (avoid abacavir) + tag genotype + provenance; LD-proxy (not sequence-based typing) | **VALIDATED vs the free 1000G HLA truth** (`20140702_hla_diversity`, n=1103): **sens 0.979 / spec 0.992 / PPV 0.855** — the deployed clinical abacavir screen, independently measured (`wiki/hla_validation_2026-07-06.md`). The provisional B\*58:01 (rs9263726, sens 0.61 weak) + A\*31:01 (rs1061235, not-paneled) FAILED validation → demoted to a documented negative, NOT shipped. Research-use, **NOT a clinical tool** |
