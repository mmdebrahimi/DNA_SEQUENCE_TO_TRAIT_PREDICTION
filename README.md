...
| **Decoder-suite validation report card** | `python scripts/build_validation_report_card.py` | Read-only roll-up (exit 0 always — a report, not a gate). Rows = deployed-claim surface (`dna_decode/data/shipped_decoder_surface.py`) ∪ observed cells; honest per-cell tier, no aggregate headline. Writes `wiki/decoder_validation_report_card.{md,json}`. |
