...
| `dna-amr` | antibiotic resistance (R/S) — cipro / cef / tet / gent; **E. coli + Klebsiella** | R/S call + the curated AMRFinder resistance determinants driving it (e.g. `gyrA_S83L`, `blaCTX-M-15`, `aac(3)-IIa`) + `undetectable_mechanisms` blind-spots on S calls | **cipro** E. coli N=147 acc 0.925 (cross-organism QRDR-POINT rule); **cef** N=60 0.933; **gent** N=128 0.945; **tet** N=12 0.833. **Cross-source (NCBI, zero BV-BRC overlap): cipro 1.0 / cef 0.864 / gent 1.0 / tet 0.909**, beating naive AMRFinder. **Cross-ORGANISM: Klebsiella cipro N=30 acc 1.0** (`wiki/klebsiella_cipro_transfer_2026-06-07.md`). Per-drug rules in `amr_rules.py::DRUG_RULE` |
