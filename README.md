...
| `dna-decode inverse` (**new**) | **effect → EDIT** — inverse design: propose the edits at a target **percentile of predicted molecular damage** (Regime B: enzyme fitness/stability), from a protein sequence (+ optional CDS → single-nt-reachable edits only). Uses the DMS-validated forward oracle as **label-free ground truth** — no phenotype label is consulted | k proposed edits + codon changes + each one's score-percentile, plus an explicit `does_not_support` list and a tie-group warning when the scorer is coarsely quantized | **It RANKS, it does NOT dose.** Graded **non-circularly** against MEASURED wet-lab ProteinGym DMS (calibrate/select on disjoint POSITION splits; grade on the proposed variant's measured value, never the model's own re-score): **beats an exact no-oracle null on 4/4 usable proteins across 4 kingdoms**, ~2–5 percentile points at top-5 (`wiki/forward_inverse_deployable_2026-07-17.md`). The magnitude version separately PASSES on blaTEM (+53.0%) but is **not deployable** — it needs a calibrator fit on the *target* protein's own DMS (which would make the inverse unnecessary), and calibrators cannot transfer (the assays share no scale). The **learned oracle beats plain BLOSUM62 on only 3/4**, so the blosum62 default is often right rather than a fallback; and **utility does not track forward rank** (PTEN 0.5185 earns its keep, RL40A 0.5190 does not) → per-protein check required. Regime B only — **never** clinical resistance (use `dna-decode amr`). Research use only |
