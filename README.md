...
| `dna-decode inverse` (**new**) | **effect → EDIT** — inverse design: propose the edits at a target **percentile of predicted molecular damage** (Regime B: enzyme fitness/stability), from a protein sequence (+ optional CDS → single-nt-reachable edits only). Uses the DMS-validated forward oracle as **label-free ground truth** — no phenotype label is consulted | k proposed edits + codon changes + each one's score-percentile, plus an explicit `does_not_support` list and a tie-group warning when the scorer is coarsely quantized | **It RANKS, it does NOT dose.** Graded **non-circularly** against MEASURED wet-lab ProteinGym DMS (calibrate/select on disjoint POSITION splits; grade on the proposed variant's measured value, never the model's own re-score). **The LEARNED method (ESM) beats an exact no-oracle null on 4/4 hand-picked proteins** (~2–5 percentile pts at top-5, `wiki/forward_inverse_deployable_2026-07-17.md`) — **but the SHIPPED wheel-only default (`blosum62`) does NOT generalize: across N=200 ProteinGym it beats a random pick materially on only 13.5%, and is often worse than guessing** (`wiki/proteingym_inverse_sweep_2026-07-17.md`). So the default is a cheap first pass, ESM (GPU) is what makes it reliable, and **utility does not track forward rank** (PTEN 0.5185 earns its keep, RL40A 0.5190 does not) → run the falsifier on *your* protein first. The magnitude version PASSES on blaTEM (+53.0%) but is **not deployable** (needs the target protein's own DMS; calibrators cannot transfer). Regime B only — **never** clinical resistance (use `dna-decode amr`). Research use only |
